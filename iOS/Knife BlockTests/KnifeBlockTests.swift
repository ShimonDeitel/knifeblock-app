import XCTest
@testable import KnifeBlock

@MainActor
final class KnifeBlockTests: XCTestCase {
    ...
    func testAddIncreasesCount() {
        ...
        store.add(Knife(name: "Test Item", maker: "x", steel: "x", notes: "x"))
        XCTAssertEqual(store.items.count, before + 1)
    }

    func testDeleteRemovesItem() {
        let store = Store()
        let item = Knife(name: "Deletable", maker: "x", steel: "x", notes: "x")
        store.add(item)
        store.delete(item)
    ...
    func testCannotAddMoreAtLimitWhenNotPro() {
        ...
        while store.items.count < Store.freeLimit {
            store.add(Knife(name: "Filler", maker: "x", steel: "x", notes: "x"))
        }
    ...
    func testUpdateModifiesExistingItem() {
        let store = Store()
        var item = Knife(name: "Original", maker: "x", steel: "x", notes: "x")
        store.add(item)
        item.name = "Renamed"
        store.update(item)
